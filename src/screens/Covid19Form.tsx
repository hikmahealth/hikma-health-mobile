...
const symptomValues: MapOrEntries<string, boolean> = symptomQuestionItems.map((sy) => [sy, false])
const riskFactorValues: MapOrEntries<string, boolean> = riskFactorQuestionItems.map((rf) => [
  rf,
  false,
])
...
export function Covid19Form(props: Props) {
  ...
  const [firstSymptomOnsetDate, setFirstSymptomOnsetDate] = useState(subDays(new Date(), 5))
  const [travelDeparture, setTravelDeparture] = useState(subDays(new Date(), 21))
  ...
  const emergencyResult = useMemo(() => {
    ...
    return syMap.get("chestPain") || syMap.get("confusion") || syMap.get("bluish")
  ...
  const testAndIsolate = useMemo(() => {
    ...
    return dangerSigns.some((symptom) => syMap.get(symptom) || rfMap.get(symptom))
  }, [])
    ...
    <Screen preset="scroll" style={$screen}>
      <View style={{ rowGap: 16 }}>
        ...
        <View>
          ...
          <DatePickerButton date={firstSymptomOnsetDate} onDateChange={setFirstSymptomOnsetDate} />
        </View>
        ...
        <Collapsible label={translate("riskFactors")}>
          <View style={{ rowGap: 16 }}>
            {riskFactorQuestionItems.map((symptom, index) => (
              <HorizontalRadioGroup
                ...
                label={translate(symptom as TxKeyPath)}
                onChange={(val) => rfActions.set(symptom, val === "yes" ? true : false)}
              />
            ))}
          </View>
          ...
          <If condition={rfMap.get("travel") === true}>
            <View style={{ flexDirection: "row", columnGap: 10, paddingTop: 32 }}>
              <View style={{ flex: 1 }}>
                <Text variant="labelLarge">{translate("symptomsDate")}</Text>
                <DatePickerButton date={travelDeparture} onDateChange={setTravelDeparture} />
              </View>

              <View style={{ flex: 1 }}>
                <Text variant="labelLarge">{translate("symptomsDate")}</Text>
                <DatePickerButton date={travelReturn} onDateChange={setTravelReturn} />
              </View>
            </View>
          </If>
        </Collapsible>
  ...
  return (
    <View>
      <Text style={{ fontWeight: "bold" }}>
        ...
        {!metadataObj.testAndIsolate && !metadataObj.testAndIsolate && translate("noAction")}
      </Text>
      {!!metadataObj.fever ? (
        <Text>

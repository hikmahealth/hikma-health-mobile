import { Model, Query, Relation } from "@nozbe/watermelondb"
import {
  field,
  date,
  readonly,
  text,
  relation,
  children,
  json,
} from "@nozbe/watermelondb/decorators"

import DrugCatalogue from "./DrugCatalogue"
import PrescriptionItem from "./PrescriptionItem"

export default class ClinicInventory extends Model {
  static table = "clinic_inventory"
  static associations = {
    drug_catalogue: { type: "belongs_to", key: "drug_id" },
    prescription_items: { type: "belongs_to", key: "prescription_id" },
  } as const

  @text("clinic_id") clinicId!: string
  @text("drug_id") drugId!: string
  @text("batch_number") batchNumber!: string
  @text("batch_expiry_date") batchExpiryDate!: string
  @text("batch_id") batchId!: string
  @field("quantity_available") quantityAvailable!: number
  @field("reserved_quantity") reservedQuantity!: number
  @date("last_counted_at") lastCountedAt!: Date | null
  @text("recorded_by_user_id") recordedByUserId!: string | null
  @json("metadata", sanitizeMetadata) metadata!: Record<string, any>
  @field("is_deleted") isDeleted!: boolean
  @date("deleted_at") deletedAt!: Date | null
  @date("last_modified") lastModified!: Date
  @date("server_created_at") serverCreatedAt!: Date | null
  @readonly @date("created_at") createdAt!: Date
  @readonly @date("updated_at") updatedAt!: Date

  @relation("drug_catalogue", "drug_id") drug!: Relation<DrugCatalogue>
  // @relation("prescription_items", "prescription_id") prescriptionItems!: Relation<PrescriptionItem>
}

function sanitizeMetadata(json: any) {
  if (!json) return {}
  if (typeof json === "string") {
    try {
      return JSON.parse(json)
    } catch {
      return {}
    }
  }
  return json
}

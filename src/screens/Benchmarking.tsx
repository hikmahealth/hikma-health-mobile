...
export function Benchmarking() {
  ...
  const saveData = async () => {
    const patients = Array.from({ length: patientsCount }, generatePatient).map((p) =>
      omit(p, ["id", "createdAt", "updatedAt"]),
    ) as Patient[]
    console.log(provider, clinic)
    if (provider === null || clinic === null) {
      Alert.alert("Error", "Please your account is not registered with any clinic")
      setLoading(false)
      return
    }
    ...
    const batchPatientRegistrations = patients.map((patient) => {
      ...
      return database.get<PatientModel>("patients").prepareCreate((newPatient) => {
    ...
    const res = await database.write(
      async () =>
        await database.batch([...batchPatientRegistrations, ...batchVisits, ...batchEvents]),
    )
  ...
  const submit = () => {
    ...
    Alert.alert(`${patientsCount} patients ready to save in the database. Proceed?`, "", [
      {
        text: "Cancel",
      },
      {
  ...
  return (
    <Screen preset="fixed">
      <View style={$formContainer}>
        ...
        <Text>
          Create {patientsCount.toLocaleString()} patients, each with {eventsPerPatient} events in
          their visits.
        </Text>

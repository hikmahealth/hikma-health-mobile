...
export function generateDummyPatients(
    ...
    for (let i = 0; i < patientsCount; i++) {
        const patient = {
            ...
            dateOfBirth: format(faker.date.birthdate({ min: 1960, max: new Date().getFullYear() }), "yyyy-MM-dd"),
...
export async function insertBenchmarkingData(data: PatientBenchmarkingData): Promise<void> {
    await database.write(async () => {
        ...
        for await (const entry of data) {
            ...
            // create the patient
            let pq = await database.collections.get<PatientModel>("patients").prepareCreate(newPatient => {
            ...
            for await (const visit of visits) {
                const vq = await database.get<VisitModel>("visits").prepareCreate((newVisit) => {
                    ...
                    newVisit.checkInTimestamp = isValid(visit.checkInTimestamp) ? new Date(visit.checkInTimestamp) : new Date()
        ...
        await database.batch(
            [...patientQuery, ...visitQuery, ...eventQuery]
        )
    })
}